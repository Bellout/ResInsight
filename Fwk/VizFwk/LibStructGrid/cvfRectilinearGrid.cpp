...
Vec3d RectilinearGrid::cellCentroid(size_t cellIndex) const
{
  ...
  Vec3d v(m_iCoordinates[i],
          m_jCoordinates[j],
          m_kCoordinates[k]);

  v += Vec3d(m_iCoordinates[i + 1],
             m_jCoordinates[j + 1],
             m_kCoordinates[k + 1]);
...
///
//--------------------------------------------------------------------------------------------------
void RectilinearGrid::cellMinMaxCordinates(size_t cellIndex,
                                           Vec3d* minCoordinate,
  ...
  minCoordinate->set(m_iCoordinates[i],
                     m_jCoordinates[j],
                     m_kCoordinates[k]);

  maxCoordinate->set(m_iCoordinates[i + 1],
                     m_jCoordinates[j + 1],
                     m_kCoordinates[k + 1]);
}
...
///
/// The average scalar value is computed as an unweighted
/// average of the cell center scalar values of all the
/// cells that are sharing the specified grid point.
///
//--------------------------------------------------------------------------------------------------
...
///
//--------------------------------------------------------------------------------------------------
bool RectilinearGrid::pointScalar(uint scalarSetIndex,
                                  const Vec3d& p,
...
/// same scalarSetIndex
//--------------------------------------------------------------------------------------------------
void RectilinearGrid::setGridPointScalarSet(uint scalarSetIndex,
                                            DoubleArray* gridPointScalarValues )
{
  CVF_ASSERT(scalarSetIndex < scalarSetCount());
