...
//==========================================================
/// Class to keep (principally static) gui presentation
/// information of a data structure item (field or object)
/// used by PdmUiItem
//==========================================================
...
class PdmUiItemInfo
{
 public:
  ...
  PdmUiItemInfo(const QString& uiName, QIcon icon = QIcon(),
                QString toolTip = "", QString whatsThis = "",
                QString extraDebugText = "")
    : m_uiName(uiName), m_icon(icon), m_toolTip(toolTip),
      m_whatsThis(whatsThis), m_extraDebugText(extraDebugText),
      m_editorTypeName(""), m_isHidden(false),
      m_isTreeChildrenHidden(false), m_isReadOnly(false),
      m_labelAlignment(LEFT)
  { }
 ...
 private:
  ...
  ///< Use this exact type of editor to edit this UiItem
  QString m_editorTypeName;

  ///< UiItem should be hidden. -1 means not set
  int m_isHidden;

  ///< Children of UiItem should be hidden. -1 means not set
  int m_isTreeChildrenHidden;

  ///< UiItem should be insensitive, or read only. -1 means not set.
  int m_isReadOnly;
...
class PdmUiItem
{
 ...
 public: // Pdm-Private only
  // =========================================================
  /// This method sets the GUI description pointer, which
  /// is supposed to be statically allocated somewhere. the
  /// PdmGuiEntry class will not delete it in any way, and
  /// always trust it to be present.
