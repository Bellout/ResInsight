...
class AppEnum
{
 ...
 private:
  //========================================================
  /// The setup method is supposed to be specialized for
  /// each and every type instantiation of this class, and
  /// is supposed to set up the mapping between enum values,
  /// text and ui-text using the \m addItem method. It may
  /// also set a default value using \m setDefault
  //========================================================
  static void setUp();
  ...
  //========================================================
  /// A private class to handle the instance of the mapping
  /// vector. All access methods could have been placed
  /// directly in the \class AppEnum class but AppEnum
  /// implementation gets nicer this way. The real core
  /// of this class is the vector map member and the
  ...
  class EnumMapper
  {
   ...
   public:
    ...
    QString uiText(T value) const {
      size_t idx;
      for(idx = 0; idx < m_mapping.size(); ++idx) {
        if (value == m_mapping[idx].m_enumVal) {
          return m_mapping[idx].m_uiText;
        }
      }
      return "";
    }
...
// =========================================================
/// Implementation of stream operators to make
/// PdmField<AppEnum<> > work smoothly
