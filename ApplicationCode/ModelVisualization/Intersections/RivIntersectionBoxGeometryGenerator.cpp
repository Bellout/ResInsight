...
// ---------------------------------------------------------------
bool
RivIntersectionBoxGeometryGenerator::isAnyGeometryPresent() const {
...
RivIntersectionBoxGeometryGenerator::generateSurface() {
...
// ---------------------------------------------------------------
cvf::ref<cvf::DrawableGeo>
RivIntersectionBoxGeometryGenerator::createMeshDrawable() {
...
RivIntersectionBoxGeometryGenerator::triangleToCellIndex() const {
...
class Box
{
 public:
  ...
  // -------------------------------------------------------------
  Box(const cvf::Mat4d& origin,
      const cvf::Vec3d& size)
  ...
  std::array<cvf::Plane, 6> planes() {
    ...
    // -----------------------------------------------------------
    boxPlanes[FaceType::POS_I].setFromPointAndNormal(
        m_origin.translation() + m_size, cvf::Vec3d(m_origin.col(0)));

    boxPlanes[FaceType::NEG_I].setFromPointAndNormal(
        m_origin.translation()         , -cvf::Vec3d(m_origin.col(0)));

    boxPlanes[FaceType::POS_J].setFromPointAndNormal(
        m_origin.translation() + m_size, cvf::Vec3d(m_origin.col(1)));

    boxPlanes[FaceType::NEG_J].setFromPointAndNormal(
        m_origin.translation()         , -cvf::Vec3d(m_origin.col(1)));

    boxPlanes[FaceType::POS_K].setFromPointAndNormal(
        m_origin.translation() + m_size, cvf::Vec3d(m_origin.col(2)));

    boxPlanes[FaceType::NEG_K].setFromPointAndNormal(
        m_origin.translation()         , -cvf::Vec3d(m_origin.col(2)));
...
// ---------------------------------------------------------------
RimIntersectionBox*
RivIntersectionBoxGeometryGenerator::intersectionBox() const {
...
void RivIntersectionBoxGeometryGenerator::calculateArrays() {
  ...
  // -------------------------------------------------------------
  Box box(m_intersectionBoxDefinition->boxOrigin(),
          m_intersectionBoxDefinition->boxSize());
  std::array<cvf::Plane, 6> boxPlanes = box.planes();

  // -------------------------------------------------------------
  RimIntersectionBox::SinglePlaneState
      singlePlane = m_intersectionBoxDefinition->singlePlaneState();

  // -------------------------------------------------------------
  int startFace = 0; int endFace = 5;

  if (singlePlane == RimIntersectionBox::PLANE_STATE_X)
    startFace = endFace = Box::FaceType::POS_I;

  if (singlePlane == RimIntersectionBox::PLANE_STATE_Y)
    startFace = endFace = Box::FaceType::POS_J;

  if (singlePlane == RimIntersectionBox::PLANE_STATE_Z)
    startFace = endFace = Box::FaceType::POS_K;
  ...
  for (int faceIdx = startFace; faceIdx <= endFace; ++faceIdx) {
    ...
    std::array<Box::FaceType, 4> clipFaces
        = box.adjacentFaces((Box::FaceType)faceIdx);
    ...
    // -----------------------------------------------------------
    std::array<cvf::Vec3d, 4>
        faceCorners = box.faceCorners((Box::FaceType)faceIdx);
    ...
    for(size_t cccIdx = 0; cccIdx < columnCellCandidates.size(); ++cccIdx) {
      ...
      for (caf::HexGridIntersectionTools::ClipVx&
            clvx : clippedTriangleVxes_once) {
        if (!clvx.isVxIdsNative) {
          clvx.derivedVxLevel = 0;
        }
      }
      ...
      caf::HexGridIntersectionTools::
      clipTrianglesBetweenTwoParallelPlanes(clippedTriangleVxes_once,
                                            cellFaceForEachClippedTriangleEdge_once,
                                            p3Plane, p4Plane,
                                            &clippedTriangleVxes,
      ...
      for (caf::HexGridIntersectionTools::ClipVx& clvx : clippedTriangleVxes) {
        if (!clvx.isVxIdsNative && clvx.derivedVxLevel == -1) {
          clvx.derivedVxLevel = 1;
        }
      }
        ...
        for(int i = 0; i < 3; ++i) {
          ...
          if(cvx.isVxIdsNative) {
            ...
            if(cvx1.isVxIdsNative && cvx2.isVxIdsNative) {
              ...
              // -------------------------------------------------
              CVF_TIGHT_ASSERT(cvx11.isVxIdsNative
                                   && cvx12.isVxIdsNative
                                   && cvx21.isVxIdsNative
                                   && cvx22.isVxIdsNative);
              ...
              m_triVxToCellCornerWeights.push_back(
                  RivIntersectionVertexWeights(
                      cvx11.clippedEdgeVx1Id, cvx11.clippedEdgeVx2Id, cvx11.normDistFromEdgeVx1,
