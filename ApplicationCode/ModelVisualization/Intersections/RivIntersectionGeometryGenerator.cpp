...
cvf::Mat4d calculateSectionLocalFlatteningCS(const cvf::Vec3d& p1,
                                             const cvf::Vec3d& p2,
...
// ---------------------------------------------------------------
void
RivIntersectionGeometryGenerator::calculateSegementTransformPrLinePoint() {
  ...
  // -------------------------------------------------------------
  double previousSectionFlattenedEndPosX
      = m_horizontalLengthAlongWellToPolylineStart;
  ...
  for ( size_t pLineIdx = 0; pLineIdx < m_polyLines.size(); ++pLineIdx ) {
    ...
    while ( lIdx < pointCount ) {
      ...
      size_t idxToNextP
          = indexToNextValidPoint(polyLine, m_extrusionDirection, lIdx);
      ...
      if (m_isFlattened) {
        ...
        // -------------------------------------------------------
        cvf::Mat4d sectionLocalCS
            = calculateSectionLocalFlatteningCS(p1, p2, m_extrusionDirection);

        // -------------------------------------------------------
        if ( pLineIdx == 0 && lIdx == 0 ) {
          previousSectionOrigo = sectionLocalCS.translation();
        }

        // -------------------------------------------------------
        previousSectionFlattenedEndPosX
            += (sectionLocalCS.translation() - previousSectionOrigo).length();
...
void RivIntersectionGeometryGenerator::calculateArrays() {
  ...
  for (size_t pLineIdx = 0; pLineIdx < m_polyLines.size(); ++pLineIdx) {
    ...
    while ( lIdx < lineCount - 1) {
      ...
      // ---------------------------------------------------------
      size_t idxToNextP = indexToNextValidPoint(polyLine,
...
// -------------------------------------------------------------
// Find the next point in the polyline that avoids making
// the line nearly parallel to the extrusion direction
// Returns size_t(-1) if no point is found
size_t
RivIntersectionGeometryGenerator::indexToNextValidPoint(
...
// -------------------------------------------------------------
cvf::Mat4d
RivIntersectionGeometryGenerator::unflattenTransformMatrix(
    const cvf::Vec3d& intersectionPointUtm) {
