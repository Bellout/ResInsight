...
// ---------------------------------------------------------------
void RivGridBoxGenerator::
setGridBoxDomainCoordBoundingBox(const cvf::BoundingBox& bb) {

  double expandFactor = 0.05;

  // Use ScalarMapperDiscreteLinear to find human readable tick
  // mark positions for grid box sub division coordinate values

  // Expand the range for ScalarMapperDiscreteLinear until the
  // geometry bounding box has a generated tick mark coords both
  // below minimum and above maximum bounding box coords
  ...
  // -------------------------------------------------------------
  {
    ...
    while (!majorTickValuesCoversDomainValues) {
      ...
      // ---------------------------------------------------------
      if (m_domainCoordsXValues[m_domainCoordsXValues.size() - 1]
          < bb.max().x()) {
        min.x() = min.x() - bb.extent().x() * expandFactor;
        max.x() = max.x() + bb.extent().x() * expandFactor;
        majorTickValuesCoversDomainValues = false;
  ...
  // -------------------------------------------------------------
  {
    ...
    while (!majorTickValuesCoversDomainValues) {
      ...
      // ---------------------------------------------------------
      if (m_domainCoordsYValues[m_domainCoordsYValues.size() - 1]
          < bb.max().y()) {
        min.y() = min.y() - bb.extent().y() * expandFactor;
        max.y() = max.y() + bb.extent().y() * expandFactor;
        majorTickValuesCoversDomainValues = false;
  ...
  // -------------------------------------------------------------
  {
    ...
    while (!majorTickValuesCoversDomainValues) {
      ...
      // ---------------------------------------------------------
      if (m_domainCoordsZValues[m_domainCoordsZValues.size() - 1]
          < bb.max().z()) {
        min.z() = min.z() - bb.extent().z() * expandFactor;
        max.z() = max.z() + bb.extent().z() * expandFactor;
        majorTickValuesCoversDomainValues = false;
...
void RivGridBoxGenerator::createGridBoxFaceParts() {
  ...
  for (int face = POS_X; face <= NEG_Z; face++) {
...

    {
      ...
      geo->addPrimitiveSet(
          new cvf::PrimitiveSetIndexedUInt(cvf::PT_LINES,
                                           ee.lineIndices().p()));
...
// ---------------------------------------------------------------
void RivGridBoxGenerator::createLegend(EdgeType edge,
                                       cvf::Collection<cvf::Part>* parts) {
  ...
  // -------------------------------------------------------------
  float tickLength =
      static_cast<float>(m_displayCoordsBoundingBox.extent().length() / 100.0);
...

  {
    ...
    // -----------------------------------------------------------
    cvf::ref<cvf::PrimitiveSetIndexedUInt> primSet =
        new cvf::PrimitiveSetIndexedUInt(cvf::PT_LINES);
...
// ---------------------------------------------------------------
cvf::Vec3f RivGridBoxGenerator::cornerDirection(FaceType face1,
                                                FaceType face2) {
...
// ---------------------------------------------------------------
void RivGridBoxGenerator::
updateFromBackgroundColor(const cvf::Color3f& backgroundColor) {
...
void RivGridBoxGenerator::computeDisplayCoords() {
  ...
  for (size_t i = 0; i < m_domainCoordsXValues.size(); i++) {
    m_displayCoordsXValues
        .push_back(m_domainCoordsXValues[i] - m_displayModelOffset.x());
  }

  // -------------------------------------------------------------
  for (size_t i = 0; i < m_domainCoordsYValues.size(); i++) {
    m_displayCoordsYValues
        .push_back(m_domainCoordsYValues[i] - m_displayModelOffset.y());
  }

  // -------------------------------------------------------------
  for (size_t i = 0; i < m_domainCoordsZValues.size(); i++) {
    m_displayCoordsZValues
        .push_back(m_scaleZ * (m_domainCoordsZValues[i] - m_displayModelOffset.z()));
  }
}
