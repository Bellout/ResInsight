...
RiuMainWindow::RiuMainWindow()
  ...
  connect(m_mdiArea,
          ...
          SLOT(slotSubWindowActivated(QMdiSubWindow*)));
  ...
  // When enableUndoCommandSystem is set false, all commands
  // are executed and deleted immediately
...
void RiuMainWindow::cleanupGuiCaseClose() {
  ...
  for (size_t i = 0; i < additionalProjectViews.size(); i++) {
    RiuProjectAndPropertyView* projPropView =
        dynamic_cast<RiuProjectAndPropertyView*>(additionalProjectViews[i]->widget());
...
*/
  ...
  // -------------------------------------------------------------
  {
    ...
    dockPanel->setAllowedAreas(Qt::LeftDockWidgetArea
