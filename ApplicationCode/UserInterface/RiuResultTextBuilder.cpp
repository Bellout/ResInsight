...
// -----------------------------------------------------------
void RiuResultTextBuilder::setIntersectionPoint(
    cvf::Vec3d intersectionPoint) {
  m_intersectionPoint = intersectionPoint;
}

// -----------------------------------------------------------
void RiuResultTextBuilder::set2dIntersectionView(
    Rim2dIntersectionView* intersectionView) {
  m_2dIntersectionView = intersectionView;
}

// -----------------------------------------------------------
void RiuResultTextBuilder::setFace(
...
// -----------------------------------------------------------------
QString
RiuResultTextBuilder::geometrySelectionText(QString itemSeparator) {

  QString text, centerData, centroidData, vertexData;

  // ---------------------------------------------------------------
  if (m_reservoirView && m_reservoirView->eclipseCase()) {

    // -------------------------------------------------------------
    const RigEclipseCaseData* eclipseCase =
    ...
    if (eclipseCase) {
      ...
      if (m_cellIndex != cvf::UNDEFINED_SIZE_T) {
        ...
        if (grid->ijkFromCellIndex(m_cellIndex, &i, &j, &k)) {
          ...
          grid->cellCornerVertices(m_cellIndex, hc.data());
          for(size_t ii=0; ii<hc.size(); ++ii) {
            // -------------------------------------------------------
            vertexData.sprintf("Vertex[%d] :  [ %.3f, %.3f, %.3f ]\n",
                               (int)ii, hc[ii].x(), hc[ii].y(), hc[ii].z());
            text += vertexData;
          }
          text += "\n---------------------------------------------\n";
        }
      ...
      if (m_intersectionPoint != cvf::Vec3d::UNDEFINED) {
        ...
        cvf::ref<caf::DisplayCoordTransform>
            transForm = m_reservoirView->displayCoordTransform();

        cvf::Vec3d domainCoord =
            transForm->translateToDomainCoord(m_intersectionPoint);
        ...
        if (m_2dIntersectionView) {
          ...
          // ---------------------------------------------------------
          formattedText.sprintf("Horizontal length from well start: %.2f",
                                m_intersectionPoint.x());
          text += formattedText + itemSeparator;

          // ---------------------------------------------------------
          cvf::Mat4d t =
              m_2dIntersectionView->flatIntersectionPartMgr()
                  ->unflattenTransformMatrix(m_intersectionPoint);
          ...
          // ---------------------------------------------------------
          formattedText.sprintf("Intersection point : [E: %.2f, N: %.2f, Depth: %.2f]",
                                domainCoord.x(), domainCoord.y(), -domainCoord.z());
          text += formattedText;
        }
      }
    }
...
QString RiuResultTextBuilder::faultResultDetails() {
  ...
  if (m_reservoirView->eclipseCase()
    ...
    const RigFault* fault =
        mainGrid->findFaultFromCellIndexAndCellFace(m_cellIndex, m_face);
