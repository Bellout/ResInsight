...
double RigActiveCellsResultAccessor::cellScalar(size_t gridLocalCellIndex) const
{
  ...
  // Because some static results might lack LGR data
  CVF_TIGHT_ASSERT(resultValueIndex < m_activeCellInfo->reservoirActiveCellCount());
...
//--------------------------------------------------------------------------------------------------
double
RigActiveCellsResultAccessor::cellFaceScalarGlobIdx(size_t globCellIndex,
                                                    cvf::StructGridInterface::FaceType faceId) const {
